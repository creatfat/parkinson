<?php
        ...
        //fetch patient data through database
        $sql = "SELECT the.therapyID, the.User_IDpatient, the.TherapyList_IDtherapylist, user.username, user.Lat, user.Long, o.name as organization, thl.name as thl_name, thl.Dosage, med.name as med_name, n.note FROM therapy the LEFT JOIN user ON the.User_IDpatient = user.userID LEFT JOIN organization o ON o.organizationID = user.Organization LEFT JOIN therapy_list thl ON thl.therapy_listID = the.TherapyList_IDtherapylist LEFT JOIN medicine med ON med.medicineID = thl.Medicine_IDmedicine LEFT JOIN note n ON n.User_IDmed = the.User_IDmed";
       
        $result = $conn->query($sql);
        $patients_data = array();
        
        //put result of select in $patients_data 
        while($row = $result->fetch_assoc()){
          
          
          $therapyID = $row['therapyID'];
          $index = array('note'=>$row['note'], 'therapyID'=>$row['therapyID'], 'patient_name' => $row['username'], 'org' => $row['organization'], 'therapy' => $row['thl_name'], 'dosage' => $row['Dosage'], 'medicine'=>$row['med_name']);
